...
MANUAL_FIELDS = [
    ...
    {
        "key": "mcap_gdp_ratio",
        "label": "MCap/GDP Ratio (raw %)",
        "type": "number",
        "min": 10.0, "max": 400.0, "step": 0.01, "default": 90.0,
        "help": "Raw MCap/GDP percentage from GuruFocus (e.g. 242.75). Compute auto-converts to 20Y percentile.",
        "cadence": "Monthly",
        "source_url": "https://www.gurufocus.com/economic_indicators/4324/india-ratio-of-total-market-cap-over-gdp",
        "source_label": "GuruFocus India MCap/GDP (raw %)",
    },
    {
        "key": "aaa_spread_bps",
        "label": "AAA-GSec Spread (basis points)",
        "type": "number",
        "min": 0, "max": 500, "step": 1, "default": 75,
        "help": "Raw AAA-GSec spread in bps (e.g. 35). Source shows decimal % (e.g. 0.35) — multiply by 100. Compute auto-converts to 5Y percentile.",
        "cadence": "Monthly",
        "source_url": "https://indiamacroindicators.co.in/economic-indicators/10-year-credit-spread-aaa-rated-bonds-g-sec",
        "source_label": "India Macro AAA spread (% shown \u00d7100 = bps)",
    },
    {
    ...
    },
    {
        ...
        "source_label": "Trendlyne Nifty 500 > 200SMA screener (count / 5 = %)",
    },
    {
        "key": "fii_latest_month",
        "label": "FII Latest Month Net Flow (Cr)",
        "type": "number",
        "min": -200000.0, "max": 200000.0, "step": 100.0, "default": 0.0,
        "help": "Single latest month's FII net flow in Cr (e.g. -60850 for April 2026). Compute uses as 30D proxy.",
        "cadence": "Monthly",
        "source_url": "https://trendlyne.com/macro-data/fii-dii/latest/cash-pastmonth/",
        "source_label": "Trendlyne FII Last 30 Days (read FII column)",
    },
    {
        "key": "dii_latest_month",
        "label": "DII Latest Month Net Flow (Cr)",
        "type": "number",
        "min": -100000.0, "max": 200000.0, "step": 100.0, "default": 30000.0,
        "help": "Single latest month's DII net flow in Cr (e.g. 52566). Compute uses as 30D proxy.",
        "cadence": "Monthly",
        "source_url": "https://trendlyne.com/macro-data/fii-dii/latest/cash-pastmonth/",
        "source_label": "Trendlyne DII Last 30 Days (read DII column)",
    },
    {
        "key": "sip_yoy",
        "label": "SIP YoY (%)",
    ...
    },
    {
        ...
        "help": "Latest CPI YoY % (e.g. 3.8). Compute auto-derives 3M direction from past saves.",
...
COMPONENT_GROUPS = [
    ("Valuation (C1)",       ["nifty_pe_ttm", "nifty_pe_pctile", "mcap_gdp_ratio"]),
    ("Credit & Rates (C2)",  ["aaa_spread_bps", "credit_growth_yoy"]),
    ("Trend & Breadth (C3)", ["pct_above_200dma"]),
    ("Flows (C5)",           ["fii_latest_month", "dii_latest_month", "sip_yoy"]),
    ("Macro India (C6)",     ["rbi_stance", "cpi_yoy_latest", "pmi_mfg", "gst_yoy"]),
]
